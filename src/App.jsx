...
function App() {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      await axios.post("https://mern-crud-six.vercel.app/updateBook", editedBook)
  ...
  const fetchBooks = async () => {
    try {
      const res = await axios.get("https://mern-crud-six.vercel.app/getBooks");
      setBooks(res.data);
    } catch (error) {
      console.error(error);
    }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.post(`https://mern-crud-six.vercel.app/deleteBook/${id}`)
    } catch (error) {
      throw new Error(error)
    }
  }
